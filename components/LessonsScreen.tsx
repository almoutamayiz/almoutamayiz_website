...
const GLASS_CARD = "bg-neutral-900 border border-white/5 shadow-2xl hover:border-brand/30 active:scale-[0.97] transition-all duration-300 cursor-pointer overflow-hidden relative group clickable";
            ...
            <div key={step} className="section-entry px-6">
                    ...
                    <div className="max-w-2xl mx-auto w-full pt-4">
                        ...
                        <div className="grid gap-3">
                            {loading ? <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-brand" size={40} /></div> : lessons.map((lesson, idx) => {
                                ...
                                return (
                                    <div key={lesson.id} onClick={() => { setActiveLesson(lesson); setStep('lesson_content'); playClickSound(); }} className={`bg-neutral-900 p-6 rounded-3xl border border-white/5 flex items-center gap-5 hover:border-brand/40 transition-all cursor-pointer group clickable ${isDone ? 'opacity-80' : ''}`}>
                ...
                {step === 'exams_hub' && (
                    <div className="max-w-2xl mx-auto w-full pt-4">
                        ...
                        <div className="grid gap-3">
                            {exams.map((exam) => (
                                <a key={exam.id} href={exam.pdf_url} target="_blank" rel="noreferrer" className="bg-neutral-900 p-6 rounded-[2rem] border border-white/5 flex items-center justify-between hover:border-brand/40 transition-all group clickable">
