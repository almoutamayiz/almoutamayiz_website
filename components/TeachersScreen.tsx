...
const TeachersScreen: React.FC<TeachersScreenProps> = ({ user }) => {
  ...
  const fetchSubjectContext = async (subjects: string[]) => {
      ...
      try {
          ...
          if (allLessons) {
              ...
              // تقليص حجم النص إذا كان كبيراً جداً لتجنب تجاوز الحدود وتسريع المعالجة
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
      ...
      try {
          ...
          1. **الأسلوب:** كن مختصراً، مباشراً، ومحترماً. يمنع منعاً باتاً استخدام عبارات المدح المبالغ فيها. ادخل في صلب الإجابة فوراً.
          2. **المصدر الوحيد:** مرجعيتك هي "محتوى الدروس" المرفق في الأسفل فقط.
             - إذا وجدت الإجابة: استخرجها كما هي.
             - إذا لم تجد الإجابة في المحتوى: قل "عذراً، لا أملك المعلومة في الدروس المتوفرة حالياً."
          3. **السرعة:** كن موجزاً لتوفير الوقت.
          ...
          if (userMsg.image && imageFile) {
              ...
              // استخدام Gemini 2.5 Flash للسرعة القصوى في المحادثة
              const res = await ai.models.generateContent({
      ...
      <div className="fixed inset-0 z-[200] bg-black flex flex-col h-[100dvh] w-full touch-auto">
          ...
          <div className="h-16 bg-neutral-900 border-b border-white/5 flex items-center justify-between px-4 shrink-0 shadow-md relative z-10 safe-area-top">
              <div className="flex items-center gap-3">
                  <button onClick={() => setActiveSubject(null)} className="p-2 hover:bg-white/5 rounded-full text-gray-400 transition-colors">
                      <ArrowLeft size={20}/>
                  </button>
