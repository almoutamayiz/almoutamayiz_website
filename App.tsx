...
import { setGameVolume, playNotificationSound } from './utils/audio';
...
const App: React.FC = () => {
  ...
  const updateLastActive = async (userId: string) => {
      try {
          // تحديث صامت لوقت آخر ظهور (Silent update)
          await supabase.from('profiles').update({ last_active_at: new Date().toISOString() }).eq('id', userId);
      } catch (e) { console.error("Analytics Error (Active)", e); }
  };

  const trackScreenView = async (screenName: string) => {
      try {
          // استدعاء دالة RPC لزيادة العداد
          await supabase.rpc('increment_screen_view', { p_screen_name: screenName });
      } catch (e) { console.error("Analytics Error (View)", e); }
  };
  // -----------------------
  ...
  const fetchProfile = useCallback(async (userId: string, email: string, retries = 5) => {
    try {
      ...
      if (profile) {
        ...
        // --- التصحيح التلقائي للصلاحيات ---
        // إذا كان الإيميل في قائمة المدراء لكن دوره في قاعدة البيانات ليس admin
        // نقوم بترقيته فوراً في قاعدة البيانات
        const isListedAdmin = ADMIN_EMAILS.includes(cleanEmail);
        if (isListedAdmin && profile.role !== 'admin') {
            await supabase.from('profiles').update({ role: 'admin' }).eq('id', userId);
            profile.role = 'admin'; // تحديث الكائن المحلي
            console.log(`System: Auto-promoted ${cleanEmail} to admin.`);
        }
        // --------------------------------
        ...
        // Analytics: تسجيل الدخول وتحديث آخر ظهور
  ...
  useEffect(() => {
      if (gameState === GameState.APP) {
          trackScreenView(currentTab);
      }
  ...
  }, [currentTab, currentUser, gameState]);
